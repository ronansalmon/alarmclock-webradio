...
internalz = 100; // internal overall z dimension
...
module bottom() {
  difference() {
    ...
    translate([(2*wallthickness),(internalz-4*wallthickness),(2*wallthickness)])
      rotate([0,-90,0])
        intersection() {
          translate([15/2,0,0])cube(16, center=true);
          cylinder(r = cable_hole, h = cable_hole, $fn = 30);
        }
    ...
    // passe cable
    translate([internalx,lip_y-7.5,internaly+wallthickness+2*7.5])
      rotate([0,90,0])
...
module lid() {
  difference() {
    cube([actual_x,actual_y,actual_lidz+lip_overlap]);
...
bottom();
*translate([0,0,(lip_overlap + 10)-3])
  rotate([180,0,0])
    lid();
